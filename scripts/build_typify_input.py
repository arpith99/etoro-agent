...
Usage:
    python3 scripts/build_typify_input.py WORKDIR
"""
...
def render_tags_rs(domains: dict) -> str:
    lines = [
        "// Generated by scripts/build_typify_input.py -- do not edit by hand.",
        "//",
        "// One module per eToro API tag, re-exporting the schemas that tag's",
    ...
    for tag in sorted(t for t in domains if not t.startswith("_")):
        entry = domains[tag]
        if not entry["schemas"]:
            continue
        lines.append(f"/// `{tag}`")
        lines.append(f"pub mod {entry['slug']} {{")
        for name in entry["schemas"]:
            lines.append(f"    pub use crate::types::components::{name};")
        lines.append("}")
        lines.append("")
    return "\n".join(lines)
...
def main() -> int:
    ...
    TAGS_RS_PATH.write_text(render_tags_rs(domains))
    print(f"Wrote {TAGS_RS_PATH}")
    return 0
